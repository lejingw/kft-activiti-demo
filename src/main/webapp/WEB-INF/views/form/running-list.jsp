...
<body>
	...
	<table>
		...
		<tr>
			...
			<td><a class="trace" href='#' pid="${p.id }" pdid="${p.processDefinitionId}" title="点击查看流程图"><%=pageContext.getAttribute("pdid")%>,<%=pageContext.getAttribute("activityId")%>,<%=ProcessDefinitionCache.getActivityName(pageContext.getAttribute("pdid").toString(), ObjectUtils.toString(pageContext.getAttribute("activityId"))) %></a></td>
			<td>${p.suspended }</td>
		</tr>
		</c:forEach>
	</table>
